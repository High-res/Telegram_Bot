...
async def documentation_handler(message: types.Message) :
    ...
    # Сертификаты
        ...
        await bot.send_message(message.from_user.id, getDataMark()['sertificates']['after_text'], reply_markup=nav.btnToSertificates)
    # Заявление на аванс
...
async def answerMessageNo(callback_query: types.CallbackQuery) :
    ...
    if callback_query.from_user.id in getChallengeActionTgId() :
        ...
        actions.addChallengeAction(callback_query.message.text, callback_query.data, callback_query.from_user.id, CloackDate(), markup.HourMinute())
        await bot.answer_callback_query(callback_query.id)
        await bot.send_message(callback_query.from_user.id, markup.challenge()['answerNo'])
   

@dp.callback_query_handler(lambda c: c.data == markup.challenge()['yes'])
async def answerMessageYes(callback_query: types.CallbackQuery) :
    if callback_query.from_user.id in getChallengeActionTgId() :
        ...
        actions.addChallengeAction(callback_query.message.text, callback_query.data, callback_query.from_user.id, CloackDate(), markup.HourMinute())
        await bot.answer_callback_query(callback_query.id)
        await bot.send_message(callback_query.from_user.id, markup.challenge()['answerYes'])
